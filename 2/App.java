...
public class App {
    ...
    static Pedido[] lerPedidos(String nomeArquivoDados) {
        Scanner arquivo = null;
        Pedido[] pedidosCadastrados = null;
        ...
        try {
            ...
            for (int i = 0; i < numPedidos; i++) {
                String linha = arquivo.nextLine();
                Pedido pedido = criarPedido(linha);
    ...
    private static Pedido criarPedido(String dados) {
        ...
        LocalDate dataDoPedido = LocalDate.parse(dadosPedido[0], formatoData);

        int formaDePagamento = Integer.parseInt(dadosPedido[1]);
        Pedido pedido = new Pedido(dataDoPedido, formaDePagamento);

        for (int i = 2; i < dadosPedido.length; i++) {
            Produto produto = pesquisarProduto(dadosPedido[i]);
    ...
    static void ordenarPedidos() {
        ...
        if (ordenador != null && comparador != null) {
            ...
            System.out.println("Algoritmo ou comparador inválido.");
        }
    }
